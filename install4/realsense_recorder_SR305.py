...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            if args.record_imgs:
                ...
                if frame_count % 1 == 0:
                    if frame_count > 10:
                        ...
                        depth_image = cv2.rectangle(depth_image,(0,0),(width,height/bunkatsu),0,cv2.FILLED)
                        depth_image = cv2.rectangle(depth_image,(0,0),(width/bunkatsu,height),0,cv2.FILLED)
                        depth_image = cv2.rectangle(depth_image,(0,(bunkatsu-1)*height/bunkatsu),(width,height),0,cv2.FILLED)
                        depth_image = cv2.rectangle(depth_image,((bunkatsu-1)*width/bunkatsu,0),(width,height),0,cv2.FILLED)
            ...
            # resize images
            rate = 0.25
            images_resize = cv2.resize(images, (int(images.shape[1]*rate), int(images.shape[0]*rate))) 
            cv2.imshow('Recorder Realsense', images_resize)
